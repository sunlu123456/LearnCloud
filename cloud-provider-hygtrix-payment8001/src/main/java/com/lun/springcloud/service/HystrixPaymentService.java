...
public class HystrixPaymentService {
    ...
    @HystrixCommand(fallbackMethod = "paymentInfo_TimeOutHandler"/*指定善后方法名*/,commandProperties = {
            /*设置自身调用超时时间的峰值（单位毫秒），峰值内可以正常运行，超过了需要有兜底的方法处埋，作服务降级fallback。*/
            @HystrixProperty(name = "execution.isolation.thread.timeoutInMilliseconds",value = "3000")
    })
    public String paymentInfo_TimeOut(Integer id)
    {
        //int age = 10/0;//报错异常同样会走善后方法
    ...
    @HystrixCommand(fallbackMethod = "paymentCircuitBreaker_fallback",commandProperties = {
            ...
            // 那么一个bucket的时间即1秒。必须符合rolling window % numberBuckets == 0，否则会抛出异常。默认10
            @HystrixProperty(name = "metrics.rollingStats.numBuckets",value = "10"),
            //一个rolling window内最小的请求数（默认20），
            // 如果设为20，那么当一个rolling window的时间内（比如说1个rolling window是10秒）收到19个请求，
            // 即使19个请求都失败，也不会触发circuit break。
            @HystrixProperty(name = "circuitBreaker.requestVolumeThreshold",value = "10"),
            //触发短路的时间值(休眠时间窗口期)，当该值设为5000时，则当触发circuit break后的5000毫秒内都会拒绝request，
            // 也就是5000毫秒后才会关闭circuit。默认5000。
            //在这个时间窗内，降级逻辑是临时的成为主逻辑，当休眠时间窗到期，断路器将进入半开状态，释放一次请求到原来的主逻辑上，
            // 如果此次请求正常返回，那么断路器将继续闭合，主逻辑恢复，如果这次请求依然有问题，断路器继续进入打开状态，休眠时间窗重新计时。
            @HystrixProperty(name = "circuitBreaker.sleepWindowInMilliseconds",value = "10000"),
            //错误比率（百分比）阀值，如果错误率>=该值，circuit会被打开（跳闸），并短路所有请求触发fallback。（默认50）
